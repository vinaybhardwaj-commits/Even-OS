...
export const carePathwaysRouter = router({
  ...
  createTemplate: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await sql`INSERT INTO audit_logs (hospital_id, user_id, action, table_name, row_id, new_values, ip_address) VALUES (${hospitalId}, ${userId}, 'INSERT', 'pathway_templates', ${id}, ${JSON.stringify({ name: input.name })}::jsonb, 'server')`;
  ...
  updateTemplate: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const existing = await sql`SELECT id, pathway_status FROM pathway_templates WHERE id = ${input.id} AND hospital_id = ${hospitalId} LIMIT 1`;
  ...
  publishTemplate: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const existing = await sql`SELECT id, pathway_status, pathway_version FROM pathway_templates WHERE id = ${input.id} AND hospital_id = ${hospitalId} LIMIT 1`;
      if (!existing?.length) throw new TRPCError({ code: 'NOT_FOUND', message: 'Template not found' });
      ...
      const nodeCount = await sql`SELECT COUNT(*)::int as cnt FROM pathway_nodes WHERE template_id = ${input.id} AND hospital_id = ${hospitalId}`;
      ...
      await sql`INSERT INTO audit_logs (hospital_id, user_id, action, table_name, row_id, new_values, ip_address) VALUES (${hospitalId}, ${userId}, 'UPDATE', 'pathway_templates', ${input.id}, '{"action":"publish"}'::jsonb, 'server')`;
  ...
  archiveTemplate: protectedProcedure
    .input(z.object({ id: z.string().uuid() }))
    .mutation(async ({ ctx, input }) => {
      await sql`UPDATE pathway_templates SET pathway_status = 'archived', pathway_updated_at = now() WHERE id = ${input.id} AND hospital_id = ${ctx.user.hospital_id}`;
      return { success: true };
    }),
  ...
  getTemplate: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const tpl = await sql`SELECT t.*, u.name as created_by_name FROM pathway_templates t LEFT JOIN users u ON u.id = t.pathway_created_by WHERE t.id = ${input.id} AND t.hospital_id = ${hospitalId} LIMIT 1`;
      if (!tpl?.length) throw new TRPCError({ code: 'NOT_FOUND', message: 'Template not found' });

      const nodes = await sql`SELECT * FROM pathway_nodes WHERE template_id = ${input.id} AND hospital_id = ${hospitalId} ORDER BY sort_order, pn_created_at`;
      const edges = await sql`SELECT * FROM pathway_edges WHERE edge_template_id = ${input.id}`;
  ...
  addNode: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const tpl = await sql`SELECT pathway_status FROM pathway_templates WHERE id = ${input.template_id} AND hospital_id = ${hospitalId} LIMIT 1`;
      if (!tpl?.length) throw new TRPCError({ code: 'NOT_FOUND', message: 'Template not found' });
      ...
      await sql`UPDATE pathway_templates SET node_count = (SELECT COUNT(*)::int FROM pathway_nodes WHERE template_id = ${input.template_id}), pathway_updated_at = now() WHERE id = ${input.template_id}`;
  ...
  removeNode: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const node = await sql`SELECT template_id FROM pathway_nodes WHERE id = ${input.id} AND hospital_id = ${hospitalId} LIMIT 1`;
      if (!node?.length) throw new TRPCError({ code: 'NOT_FOUND', message: 'Node not found' });
      const templateId = (node as any)[0].template_id;

      await sql`DELETE FROM pathway_edges WHERE from_node_id = ${input.id} OR to_node_id = ${input.id}`;
      await sql`DELETE FROM pathway_nodes WHERE id = ${input.id}`;
      await sql`UPDATE pathway_templates SET node_count = (SELECT COUNT(*)::int FROM pathway_nodes WHERE template_id = ${templateId}), pathway_updated_at = now() WHERE id = ${templateId}`;
  ...
  activatePathway: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const tpl = await sql`SELECT id, pathway_name FROM pathway_templates WHERE id = ${input.template_id} AND hospital_id = ${hospitalId} AND pathway_status = 'active' LIMIT 1`;
      ...
      const nodes = await sql`SELECT * FROM pathway_nodes WHERE template_id = ${input.template_id} AND hospital_id = ${hospitalId} ORDER BY sort_order`;
      ...
      await sql`UPDATE care_plans SET total_milestones = ${milestoneCount} WHERE id = ${carePlanId}`;
      ...
      await sql`INSERT INTO audit_logs (hospital_id, user_id, action, table_name, row_id, new_values, ip_address) VALUES (${hospitalId}, ${userId}, 'INSERT', 'care_plans', ${carePlanId}, ${JSON.stringify({ template_id: input.template_id, milestones: milestoneCount })}::jsonb, 'server')`;
  ...
  deactivatePathway: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      await sql`UPDATE care_plans SET care_plan_status = 'revoked', revoked_at = now(), revoke_reason = ${input.reason || null}, cp_updated_at = now() WHERE id = ${input.care_plan_id} AND hospital_id = ${ctx.user.hospital_id}`;
      return { success: true };
    }),
  ...
  getCarePlan: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const milestones = await sql`SELECT * FROM care_plan_milestones WHERE care_plan_id = ${input.care_plan_id} AND hospital_id = ${hospitalId} ORDER BY ms_sort_order, ms_created_at`;
  ...
  completeMilestone: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const ms = await sql`SELECT care_plan_id FROM care_plan_milestones WHERE id = ${input.milestone_id} AND hospital_id = ${hospitalId} LIMIT 1`;
      if (!ms?.length) throw new TRPCError({ code: 'NOT_FOUND', message: 'Milestone not found' });

      await sql`UPDATE care_plan_milestones SET ms_status = 'completed', ms_completed_at = now(), ms_completed_by = ${userId}, ms_updated_at = now() WHERE id = ${input.milestone_id}`;

      const carePlanId = (ms as any)[0].care_plan_id;
      await sql`UPDATE care_plans SET completed_milestones = (SELECT COUNT(*)::int FROM care_plan_milestones WHERE care_plan_id = ${carePlanId} AND ms_status = 'completed'), cp_updated_at = now() WHERE id = ${carePlanId}`;
  ...
  skipMilestone: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const ms = await sql`SELECT care_plan_id, ms_patient_id, due_datetime FROM care_plan_milestones WHERE id = ${input.milestone_id} AND hospital_id = ${hospitalId} LIMIT 1`;
      if (!ms?.length) throw new TRPCError({ code: 'NOT_FOUND', message: 'Milestone not found' });
      const m = (ms as any)[0];

      await sql`UPDATE care_plan_milestones SET ms_status = 'skipped', skipped_at = now(), skip_reason = ${input.skip_reason}, skipped_by = ${userId}, ms_updated_at = now() WHERE id = ${input.milestone_id}`;
  ...
  startMilestone: protectedProcedure
    .input(z.object({ milestone_id: z.string().uuid() }))
    .mutation(async ({ ctx, input }) => {
      await sql`UPDATE care_plan_milestones SET ms_status = 'in_progress', ms_started_at = now(), ms_updated_at = now() WHERE id = ${input.milestone_id} AND hospital_id = ${ctx.user.hospital_id}`;
      return { success: true };
    }),
  ...
  getMilestoneDetail: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const ms = await sql`SELECT * FROM care_plan_milestones WHERE id = ${input.milestone_id} AND hospital_id = ${hospitalId} LIMIT 1`;
      if (!ms?.length) throw new TRPCError({ code: 'NOT_FOUND', message: 'Milestone not found' });

      const variances = await sql`SELECT * FROM variance_log WHERE vl_milestone_id = ${input.milestone_id} AND hospital_id = ${hospitalId} ORDER BY vl_created_at DESC`;
      const escalations = await sql`SELECT * FROM escalation_events WHERE ee_milestone_id = ${input.milestone_id} AND hospital_id = ${hospitalId} ORDER BY ee_created_at DESC`;
  ...
  recordVariance: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const ms = await sql`SELECT care_plan_id, ms_patient_id FROM care_plan_milestones WHERE id = ${input.milestone_id} AND hospital_id = ${hospitalId} LIMIT 1`;
      if (!ms?.length) throw new TRPCError({ code: 'NOT_FOUND', message: 'Milestone not found' });
      const m = (ms as any)[0];
  ...
  createEscalation: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await sql`UPDATE care_plans SET overdue_milestones = (SELECT COUNT(*)::int FROM care_plan_milestones WHERE care_plan_id = ${m.care_plan_id} AND ms_status IN ('not_started','in_progress') AND due_datetime < now()), cp_updated_at = now() WHERE id = ${m.care_plan_id}`;
  ...
  acknowledgeEscalation: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      await sql`UPDATE escalation_events SET ee_status = 'acknowledged', acknowledged_at = now(), acknowledged_by = ${ctx.user.sub} WHERE id = ${input.escalation_id} AND hospital_id = ${ctx.user.hospital_id}`;
      return { success: true };
    }),
  ...
  resolveEscalation: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      await sql`UPDATE escalation_events SET ee_status = 'resolved', ee_resolved_at = now(), ee_resolved_by = ${ctx.user.sub}, resolution_notes = ${input.resolution_notes || null} WHERE id = ${input.escalation_id} AND hospital_id = ${ctx.user.hospital_id}`;
      return { success: true };
    }),
  ...
  pathwayStats: protectedProcedure
    .query(async ({ ctx }) => {
      ...
      const templates = await sql`SELECT COUNT(*)::int as cnt FROM pathway_templates WHERE hospital_id = ${hospitalId} AND pathway_status = 'active'`;
      const activePlans = await sql`SELECT COUNT(*)::int as cnt FROM care_plans WHERE hospital_id = ${hospitalId} AND care_plan_status = 'active'`;
