 ...
 *  - scripts/verify-admin-manifest.ts — CI gate (added in AD.2)
 *
 * Drift prevention: the CI gate in AD.2 will fail the build if a page exists
 * under src/app/(admin)/admin/** without a manifest entry here. That is how
 * we avoid ever repeating the "50 pages unreachable" mess again.
 *
 * This file is the AD.1 stub — it covers ~25 representative routes across the
 * 8 pillars so the shell is usable. AD.2 will enumerate all ~88 admin pages.
 */
